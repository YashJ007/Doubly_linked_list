...
printf("\nEnter data at start:\n");
scanf("%d", &head->data); head->next = NULL; head->prev = NULL;
}
...
temp->data = input;
    temp->prev = NULL; temp->next = head; head->prev = temp; head = temp; return head;
}
...
struct node *delete_start(struct node *head) {
 struct node *p;
p = head;
head = head->next; head->prev = NULL; free(p);
return head; }
...
scanf("%d", &menucontrol); switch (menucontrol)
